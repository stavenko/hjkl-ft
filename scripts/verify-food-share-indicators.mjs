...
const plankaDays = payload?.food?.planka_days || [];
const calorieInd = inds.find((i) => i.label === "Калории");
console.log("indicators in share:", inds.map((i) => `${i.label}:${i.state}`).join(", "));
console.log("planka_days:", JSON.stringify(plankaDays));
...
const ok = !!calorieInd && plankaDays.length > 0 && inds.length >= 3;
console.log(ok ? `\n✅ PASS — food share carries the indicators (Калории=${calorieInd?.state}) + planka adherence` : "\n❌ FAIL");
process.exit(ok ? 0 : 1);
