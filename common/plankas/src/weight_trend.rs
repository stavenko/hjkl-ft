...
/// Дневные средние по окну. Публична, потому что ею пользуется анализ цикла в
/// приложении худеющего: то же сглаживание, что и у тренда, и вторая его копия
/// разошлась бы с первой.
pub fn daily_means(entries: &[WeightEntry], window_days: i64) -> Vec<(f64, f64)> {
...
/// Неполная бета-функция — хвост распределения Стьюдента. Публична по той же
/// причине, что `daily_means`.
