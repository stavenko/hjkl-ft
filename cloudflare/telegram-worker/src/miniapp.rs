...
pub async fn miniapp_me(mut req: Request, env: &Env) -> Result<Response> {
    ...
    for claim in claims {
        ...
        if status == "paid" || status == "claimed" {
            ...
            // Ссылка «Открыть приложение» для уже зарегистрированного человека: корень
            // приложения, но С АККАУНТОМ — `/?u=<user_id>`. По ней сразу ставится
            // ярлык: страница отдаётся с ПЕРСОНАЛЬНЫМ манифестом (`_worker.js`
            // подставляет `?u` в <link rel=manifest>), и «На экран „Домой“» снимает
            // манифест со `start_url:/?u=<user_id>` — поставленное приложение знает,
            // в какой аккаунт входить. Корень БЕЗ `?u` давал общий манифест со
            // `start_url:"/"`, и ярлык терял пользователя. Онбординг здесь ни при чём:
            // человеку нужна установка, а не регистрация. Без user_id (резолв не
            // удался) остаётся прежний голый корень — лучше, чем ничего.
            let root = base.strip_suffix("/onboard").unwrap_or(&base).to_string();
            let app_url = match &account_user_id {
                Some(uid) => format!("{root}/?u={uid}"),
                None => root,
            };
