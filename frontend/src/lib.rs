...
async fn sync_at_launch() {
    // Сверка с сервером — ДО всякой проверки связи: расхождение могло остаться с
    // прошлого запуска, и человек обязан увидеть плашку, даже если сейчас он
    // офлайн и синк вообще не начнётся.
    services::sync::refresh_in_sync().await;
...
async fn maintain_user_data() {
    ...
    // Следом красное мясо — когда закрыта неделя жиров.
    services::indicators::maybe_unlock_red_meat_week().await;
