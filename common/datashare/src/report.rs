...
pub struct Clock {
    ...
    /// Час, в который у человека начинается новый день. Сейчас это полночь, но
    /// едет он явно: правило может смениться, а куратор не обязан его знать.
    #[serde(default)]
...
///
/// Дней здесь нет и не будет. Число дней человек не знает: он знает «я уже
/// присылал» и «я ещё ничего не присылал», а перевести это в 17 не может ни он,
/// ни куратор. Поэтому выбор из двух, и оба выражены в его словах.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Scope {
    /// Всё, что накопилось ПОСЛЕ последнего отправленного отчёта.
    New,
...
impl Scope {
    ...
    /// Неизвестное значение — это `All`, а не отказ. Директива могла прийти от
    /// сборки старше этой; прислать больше нужного не вредно, а промолчать —
    /// вредно.
...
/// «Сутки клиента закончатся в полночь по его времени — 22:00 у вас, через 9 ч».
///
/// Куратор судит по датам, которых сам не считает: они пришли из другого пояса.
/// Поэтому здесь три вещи сразу — час перелома у человека, тот же момент в СВОЁМ
/// времени и сколько до него осталось. Без последнего куратор не отличит «сутки уже полные» от «человек их
...
fn day_boundary_line(clock: &Clock) -> Option<String> {
    ...
    // Полночь называем словом: «в 00:00» читается как время суток, а не как
    // граница, и куратор второй раз переспрашивает себя, что это значит.
    let theirs = clock.day_start_hour.map(|h| match h {
        0 => "полночь".to_string(),
        h => format!("{h:02}:00"),
    });
    ...
    Some(match theirs {
        Some(t) => format!("Сутки клиента заканчиваются в {t} по его времени{where_} — в {mine} у вас, {when}"),
