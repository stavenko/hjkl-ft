...
// ── Mi Browser (Xiaomi) ──────────────────────────────────────────────────────
//
// Ключ доступа там создать нельзя: `PublicKeyCredential` отсутствует вовсе
// (замерено пробником на Redmi, Android 15, MiuiBrowser 14.60). Пользоваться
// приложением невозможно, человеку надо в Chrome — и больше ничего.
//
// Поэтому Mi получает СВОЮ страницу прямо отсюда, а не экран внутри приложения:
// решение принимается до того, как отдан хоть байт, так что не грузится ни
// манифест, ни сервис-воркер, ни WASM. Ради одной кнопки качать приложение
// целиком незачем, а манифест вдобавок подбивает браузер предлагать ярлык на
// рабочий стол — ставить который тут ровно не надо.
//
// Экран `android_mi` в приложении при этом ОСТАЁТСЯ: если сервис-воркер, живущий
// с прошлых визитов, отдаст навигацию из кэша, сюда управление не попадёт, и
// показать что-то осмысленное должно само приложение. Текст в двух местах —
// плата за это; он короткий и меняется вместе.
const MI_TITLE = "Приложение re:Norma работает в браузере Chrome.";
const MI_BUTTON = "Открыть в Chrome";

function isMiBrowser(request) {
...
export default {
  async fetch(request, env) {
    ...
    if (isMiBrowser(request) && isPageRequest(request)) {
