...
#[component]
pub fn StoryCh2MistakePage() -> impl IntoView {
    let navigate = use_navigate();

    // The task is to keep the food diary 7 days in a row. Compute the
    // consecutive-day streak over distinct diary-entry dates.
    ...
    let done = move || streak.get() >= 7;
    let has_data = move || streak.get() > 0;

    let paragraphs = ["story.ch2.mistake.p1", "story.ch2.mistake.p2", "story.ch2.mistake.p3"];
    ...
    view! {
        <div style=PAGE_BG>
            ...
            <h1 class="is-size-1 has-text-weight-bold" style="margin: 0 16px 16px 16px;">{move || t("story.ch2.s1")}</h1>
            ...
            <div style="padding: 16px 16px 0 16px;">
                <p class="is-size-7 has-text-grey-light" style="text-transform: uppercase; letter-spacing: 0.02em; margin: 0 0 8px 4px;">
                    {move || t("story.ch2.mistake.task_label")}
                </p>
                <div style=CARD>
                    <div style="display: flex; align-items: flex-start; gap: 12px; padding: 14px 16px;">
                        ...
                        <span class="is-size-6 has-text-weight-semibold" style="flex: 1; line-height: 1.4;">{move || t("story.ch2.mistake.task")}</span>
                    </div>
                </div>
                ...
                {move || if has_data() {
                    view! {
                        <p class="is-size-7 has-text-grey" style="margin: 8px 0 0 4px;">
                            {move || format!("{}: {}/7", t("story.ch2.mistake.streak_label"), streak.get())}
                        </p>
                    }.into_view()
                } else {
                    view! {
                        <p class="is-size-7 has-text-grey" style="margin: 8px 0 0 4px;">
                            {move || t("story.ch2.mistake.no_data")}
                        </p>
                    }.into_view()
                }}
