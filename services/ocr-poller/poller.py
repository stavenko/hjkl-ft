...
# NOTE: there are NO prompts here on purpose. The poller is a dumb executor —
# prompts (label OCR, food-items) are built on the frontend (localized via i18n)
# and passed in with each job. See docs/food-photo-recognition.md.
...
def recognize(queue, job_id, image_blob, prompt, proxy):
    """Run the given `prompt` against the on-prem VLM and return the RAW answer
    text. The poller is a DUMB executor — no built-in prompt, no JSON parsing, no
    validation, no Atwater: all business logic lives on the frontend, which builds
    the prompt (localized via i18n) and parses this raw output."""
    try:
        ...
        images = [image_blob]
    return _run_vlm(queue, job_id, images, prompt, proxy)
...
def main():
    ...
    while True:
        ...
        try:
            ...
            result = recognize(queue, job_id, image_b64, prompt, proxy)
            with hard_deadline(COMPLETE_HARD):
                complete(queue, job_id, proxy, result=result)
            print(f"done {job_id} ({len(result)} chars): {result[:160]}", flush=True)
        except Exception as e:
