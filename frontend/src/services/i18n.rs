...
fn en(key: &str) -> &'static str {
    match key {
        ...
        "pwa.mi.title" => "re:Norma works in Chrome",
        "pwa.mi.lead" => "This browser cannot create the access key re:Norma signs you in with. Open the app in Chrome — everything works there.",
        "pwa.mi.open" => "Open in Chrome",
        "pwa.mi.fallback" => "If the button did nothing, Chrome is probably not installed. Install it and open this address:",
...
fn ru(key: &str) -> &'static str {
    match key {
        ...
        "pwa.mi.title" => "re:Norma работает в браузере Chrome",
        "pwa.mi.lead" => "Этот браузер не умеет создавать ключ доступа, которым re:Norma пускает вас внутрь. Откройте приложение в Chrome — там всё работает.",
        "pwa.mi.open" => "Открыть в Chrome",
        "pwa.mi.fallback" => "Если кнопка ничего не сделала — скорее всего, Chrome не установлен. Поставьте его и откройте адрес:",
