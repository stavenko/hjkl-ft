...
//! разбора ещё не было.
//!
//! ДОБАВИТЬ позицию в нижнюю половину нельзя, и это решение, а не недоделка. Здесь
//! правят ОТВЕТ распознавания: удаляют лишнее и уточняют вес. Дописать продукт
//! руками значило бы завести отсюда обычную еду — а для этого есть обычный путь,
//! поиск по базе, и он никуда не делся. Экран, умеющий и то и другое, перестал бы
//! отвечать на вопрос «что тут распозналось». См. `docs/lazy-diary-entry.md` §7.5:
//! у нижней половины ровно два действия, удалить и изменить вес.
