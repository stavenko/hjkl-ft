...
///
/// NOTE: distinct from lib.rs `read_secret_store`, which is Store-ONLY (no [vars]
/// fallback) because lava is intentionally prod-only. This one has the fallback so
/// shared secrets (JWT_SECRET, INTERNAL_PUSH_KEY) keep working in dev/test.
