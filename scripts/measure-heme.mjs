...
const CASES = [
  ...
  // Прочее.
  // Гем тут ЕСТЬ, и мы говорим об этом честно: по ГОСТ 23670 говядина со свининой
  // занимают около 95 % батона. Ждать «нет» было ошибкой набора, а не модели —
  // модель отвечала «да» единогласно и была права. Индикаторы существуют не для
  // того, чтобы всегда быть зелёными: съевшему колбасы действительно не нужно
  // добирать гем отдельно. То, что колбаса ПРИ ЭТОМ мясо глубокой переработки,
  // говорит свой индикатор — [`processed_meat`], и это разные разговоры.
  ["Колбаса докторская", true, "мясо млекопитающих — главная часть"],
