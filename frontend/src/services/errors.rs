...
/// Есть ли смысл переспросить ПРЯМО СЕЙЧАС — через паузу в доли секунды, не
/// выходя из запроса.
///
...
///
/// - **4xx** — не повторяем. 401 и 403 не изменятся от настойчивости, 400 и 404
///   тем более. 429 — тем более нельзя: «слишком часто» лечится паузой, а не
///   тремя запросами подряд. Исключение одно: 408 (таймаут) — там повтор и есть
///   ответ.
/// - **5xx** — повторяем: сервер прилёг, это проходит само.
...
///
/// Это НЕ то же самое, что «когда вернуться к шагу» (`ai_steps::classify`): там
/// речь про следующий проход очереди, до которого проходят минуты и часы, и туда
/// 429 как раз попадает. Один и тот же код отвечает на два вопроса по-разному,
...
mod tests {
    ...
    fn to_chto_prohodit_samo_povtoryaem() {
        ...
        // «Не успел» — повтор и есть ответ.
        assert!(worth_retrying("HTTP 408: timeout"));
        // А «слишком часто» подряд повторять НЕЛЬЗЯ: это лечится паузой. К такому
        // шагу возвращается очередь на следующем проходе (`ai_steps::classify`).
