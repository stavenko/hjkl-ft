...
//
// ЧЕГО ЭТОТ СПОСОБ ПОКА НЕ УМЕЕТ. На горке он хорош, на плоском — нет:
//
//   печень 300 г + капуста 400 г (горки)    -8% и -4%
//   скумбрия 83 г + сельдь 72 г (плоское)   +70…+127%
//
// Поперечник на снимке с рыбой почти точный (19.2 при 19), значит промах целиком в
// толщине. Арифметика показывает, где именно: 155 г рыбы это 148 см³, и на дне в
// 284 см² при покрытии в 55-65% средняя толщина выходит около 1 см. Модель же
// называет 1.8 и 2.2 см — вдвое больше.
//
// Это та же тяга к типичному, что и с поперечником посуды (она тянула всякую тарелку
// к 22 см): толщину она тянет к паре сантиметров и ниже сантиметра не опускается.
// Коэффициенты DENSITY тут ни при чём — плотность рыбы около 1.05, а solid и так
// стоит 0.90 x 1.00.
//
// И, похоже, дело не в промпте вовсе. ТОЛЩИНЫ НА СНИМКЕ СВЕРХУ ПРОСТО НЕТ. Поперечник
// посуды лежит В ПЛОСКОСТИ кадра, его можно измерить — и рамками он и измеряется, до
// +1%. Высота слоя перпендикулярна плоскости кадра, и никакой информации о ней в
// одном виде сверху не содержится: модель не измеряет её, а вспоминает типичную для
// такого блюда. На капусте с мясом типичная совпала с настоящей, на рыбе нет.
//
// Отсюда и объяснимо, почему в исследованиях по этой задаче везде депт-камера: она
// добавляет ровно ту координату, которой в кадре не хватает.
//
// Что уже пробовали против этого и что не помогло: точка отсчёта в кадре (--scale в
// measure-per-image.mjs), поле с оценкой доли тарелки перед граммами, четвёртая
// категория плотности hollow, миллиметры вместо сантиметров.
//
...
/// В каких единицах спрашивать толщину слоя. Проверено: НЕ ВАЖНО. Гипотеза была, что в
/// сантиметрах модель мыслит круглыми числами и ниже единицы не опускается, а в
/// миллиметрах ей есть куда идти. Она просто умножила свои же сантиметры на десять —
/// 3.5 см стали 35 мм, 1.2 стали 12, — и ответы совпали до грамма. Оценка у неё
/// готовая, единица только форма записи.
const THICK_UNIT = arg("thick", "cm");
...
const CASES = {
  ...
  // Верхний кусок с тёмной кожей — скумбрия, нижнее светлое филе — сельдь. Модель
  // обе называет сельдью, поэтому корни у скумбрии перечислены с оглядкой на это.
  "food-fish.jpg": {
    plate: 19,
    want: [
      { keys: ["скумбри", "с кожей", "запеч", "духов", "жарен"], grams: 83 },
      { keys: ["филе", "без кожи", "сельд"], grams: 72 },
    ],
  },
};
    ...
    regions: {
      ...
      items: {
        ...
        properties: {
          ...
          thickness_cm: THICK_UNIT === "mm" ? {
            description: "Насколько высоко еда поднимается над дном в самом типичном месте, В " +
              "МИЛЛИМЕТРАХ. Плоский ломтик и высокая горка отличаются здесь в разы, и мелкие " +
              "значения так же законны, как крупные: мери то, что видишь, не округляя к привычному.",
            type: "number",
          } : {
            description: "Насколько высоко еда поднимается над дном в самом типичном месте, в " +
...
///
/// Категорию hollow — «кусок с полостью внутри» — пробовали и убрали. Сам факт верен:
/// у взвешенной скумбрии (83 г, разрезана поперёк, внутри полость от брюшка)
/// заполнение выходит 0.39-0.52 при любых названных моделью долях, то есть половина
/// габарита пустая. Но, получив четвёртую категорию, модель записала в неё ОБА куска
/// рыбы, включая сплошное филе, и заодно подняла толщину с 1.8/0.6 до 2.5/1.8 см.
/// Ошибка не уменьшилась. Лишняя категория меняет поведение целиком, а не только там,
/// где задумано, — так же, как раньше числовая подсказка протекала в граммы.
const DENSITY = {
...
function grams(diameterCm, r) {
  ...
  const thickCm = THICK_UNIT === "mm" ? r.thickness_cm / 10 : r.thickness_cm;
  return area * r.share_of_plate * thickCm * fill * g_per_cm3;
}
...
async function main() {
  ...
  for (const [file, c] of Object.entries(CASES)) {
    ...
    for (const r of a.regions || []) {
      ...
      console.log(`  ${r.name}: доля ${r.share_of_plate}, слой ${r.thickness_cm} ${THICK_UNIT}, ${r.how_dense}` +
        `  →  ${g.toFixed(0)} г` +
