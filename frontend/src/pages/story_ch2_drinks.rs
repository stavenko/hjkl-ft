...
#[component]
pub fn StoryCh2DrinksPage() -> impl IntoView {
    let navigate = use_navigate();

    let paragraphs = ["story.ch2.drinks.p1", "story.ch2.drinks.p2"];
    let body = paragraphs.iter().map(|&key| view! {
        <p class="is-size-6" style="line-height: 1.55; margin: 0 0 14px 0;">{move || t(key)}</p>
    }).collect_view();

    // Task (a "don't"): no high-cal drink yesterday. Completion = report-ready
    // (yesterday) AND no high-cal drink in yesterday's diary. A zero-cal drink
    // logged yesterday earns an encouraging line (it's optional to log).
    ...
    let zero_cal = create_rw_signal(false);
    create_effect(move |_| {
        ...
        spawn_local(async move {
            ...
            done.set(ready && !local::high_cal_drink_on(&y).await);
            zero_cal.set(local::zero_cal_drink_on(&y).await);
        });
    });
    ...
    view! {
        <div style=PAGE_BG>
            ...
            <h1 class="is-size-1 has-text-weight-bold" style="margin: 0 16px 16px 16px;">{move || t("story.ch2.s5")}</h1>
            ...
            <div style="padding: 16px 16px 0 16px;">
                <p class="is-size-7 has-text-grey-light" style="text-transform: uppercase; letter-spacing: 0.02em; margin: 0 0 8px 4px;">
                    {move || t("story.ch2.drinks.task_label")}
                </p>
                <div style=CARD>
                    <div style="display: flex; align-items: flex-start; gap: 12px; padding: 14px 16px;">
                        ...
                        <span class="is-size-6 has-text-weight-semibold" style="flex: 1; line-height: 1.4;">{move || t("story.ch2.drinks.task")}</span>
                    </div>
                </div>
                ...
                {move || zero_cal.get().then(|| view! {
                    <p class="is-size-7 has-text-success" style="margin: 8px 0 0 4px;">
                        {move || t("story.ch2.drinks.effort")}
                    </p>
                })}
