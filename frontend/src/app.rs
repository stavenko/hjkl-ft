...
pub fn App() -> impl IntoView {
    ...
    create_effect(move |_| {
        ...
        leptos::spawn_local(async move {
            ...
            // Засев для испытаний — СРАЗУ ПОСЛЕ миграций и только на тестовом
            // домене. До миграций положенное не выживает: миграция перебирает
            // хранилища и приводит строки к нынешней схеме, и то, что тест успел
            // записать, до этого не доживает.
            crate::services::test_seed::apply().await;
            if done > 0 {
