...
///
/// Планка на тот день берётся из ИСТОРИИ — у ВСЕХ видов, а не только у подвижных:
/// куратор вправе сменить любую, и день обязан судиться по той величине, что
/// действовала тогда. Разница между видами одна: у подвижных отсутствие записи
/// означает «планки ещё не было» (см. ниже), у остальных — «работает наше правило».
async fn ratio_for(key: &str, date: &str, value: f64) -> (Option<f64>, Option<f64>) {
    use crate::services::plankas::Kind;
    let target = match Kind::from_key(key) {
        // Подвижные три ведёт приложение, и до самой первой установки планки у них
        // ПРОСТО НЕТ. Судить нечем: правила человеку не давали, а значит и нарушить
        // он его не мог. Планкой дня становится его собственный результат, то есть
        // день засчитывается как выполненный.
        //
        // Прежде здесь бралась ТЕКУЩАЯ планка, и первая же выдача перекрашивала всю
        // неделю наблюдений в перебор: человек ел 3850, планку получил на 3550 — и
        // задним числом оказался виноват в том, что ел до неё.
        Some(k) if k.is_dynamic() => local::planka_on(k.key(), date).await.unwrap_or(value),
        // Остальные девять — нормы. У них тоже своя история (куратор вправе назвать
        // число), но там, где записи нет, работает наше правило, единое во времени.
        Some(k) if has_daily_target(key) => {
            crate::services::plankas::on(k, date).await.unwrap_or(0.0)
        }
        // Недельная механика: железо, гем, омега-3, баланс жиров, красное мясо,
        // яйца. Дневной цели у них нет — столбик показывает количество, но день по
        // нему не судится.
        _ => 0.0,
    };
    if target > 0.0 {
...
/// Есть ли у этого вида СУТОЧНАЯ цель. Железо, гем, омега-3, баланс жиров,
/// красное мясо и яйца планку тоже имеют, но недельную: дневной цели у них нет, и
/// день по ним не судится.
fn has_daily_target(key: &str) -> bool {
    matches!(key, "protein" | "veg_fruit" | "steps" | "calories" | "calcium" | "fiber")
}

/// Действующая СУТОЧНАЯ цель для `key` (0 — считать пока не из чего, например
/// белок до заполнения профиля).
async fn target_for(key: &str) -> f64 {
    use crate::services::plankas::{self, Kind};
    if !has_daily_target(key) {
        return 0.0;
    }
