// Open a HEADED browser on dev, seeded so Story 4 (calcium week) shows, then
// auto-open the story and advance straight to FRAME 6 (the calcium indicator/gauge
// highlight). Leaves the window open.
import { chromium } from "playwright";
const FE = process.argv[2] || "https://renorma-fit-dev.pages.dev";
const uid = "s4-f6-" + Date.now();
...
// Advance to frame 6 (5 taps on the right advance zone).
for (let i = 0; i < 5; i++) { await page.mouse.click(300, 700); await page.waitForTimeout(450); }
console.log("READY — открыт 6-й кадр Story 4. Окно останется открытым.");
await new Promise(() => {});
