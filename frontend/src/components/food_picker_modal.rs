...
/// Shared food-picker modal: search, food list with a "+" action, "new food"
/// via the editor, and a grams modal. Used by both the diary-add and the
/// recipe-ingredient flows — the caller supplies the list, the disabled set and
/// what happens when a (food, grams) is confirmed.
#[component]
pub fn FoodPickerModal(
    ...
    // Foods picked during THIS dialog session. A food is shown as added (and its
    // "+" disabled) only if it's in `disabled_ids` (e.g. recipe ingredients) OR
    // it was picked in this session. This signal lives with the modal instance,
    // so it resets automatically when the dialog is closed and reopened — i.e.
    // re-opening the dialog lets you add the same product again.
                                ...
                                <button attr:data-testid="diary-add-weight-btn-confirm" class="button is-link"
                                    on:click={
                                        ...
                                        move |_| {
                                            ...
                                            // Mark as picked in this dialog session so it can't be
                                            // added twice until the dialog is closed (resets on close).
