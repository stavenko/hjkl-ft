...
/// Есть ли что засевать. Зовётся на запуске ДО того, как приложение решит, что
/// показать: сам засев тогда обходится дорого (миграции), а без него запуск идёт
/// прежним путём.
pub fn pending() -> bool {
    on_test_host()
        && storage()
            .and_then(|s| s.get_item(SEED_KEY).ok().flatten())
            .is_some()
}
...
///
/// Зовётся из `lib.rs` на запуске, СРАЗУ ПОСЛЕ миграций и ДО того, как приложение
/// решит, что показать. Оба условия обязательны:
///
/// * после миграций — иначе положенное не выживет;
/// * до решения о состоянии — иначе засеять подписку невозможно в принципе:
///   миграции и сам засев живут в эффекте, который на ЗАКРЫТОМ приложении
///   («проверяем подписку», «подписки нет») не запускается вовсе, и испытание
///   упирается в оверлей, который само же и должно было снять.
