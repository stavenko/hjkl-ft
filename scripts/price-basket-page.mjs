...
html = html.replace(RATION, (whole, head, id, body, tail) => {
  ...
  const pros = [
    ...
    ["Белок", `${n(sum.protein / 7)} г в день при норме ${t.proteinDay} — на дефиците уходит жир, а не мышцы.`],
    ...
    ["Железо", `${n(sum.ironAbs, 1)} мг усвоенного за неделю при норме ${n(t.ironWeek, 1)},`
      + ` из них ${portions(sum.heme / RULES.hemePortionProtein)} гемового при норме ${RULES.hemePortionsWeek}`
    ...
    ["Жировой профиль", `Ненасыщенных в ${n(sum.unsat / sum.sat, 1)} раза больше насыщенных — убираем риски сердечно-сосудистых болезней.`],
