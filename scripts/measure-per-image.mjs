...
//
// ВЕС ПОРЦИИ ПО СНИМКУ НЕ БЕРЁТСЯ НИ ОДНОЙ ИЗ ДОСТУПНЫХ МОДЕЛЕЙ. Тарелка с печенью
// в сливках (300 г) и варёной цветной капустой (400 г сырой), по прогону на модель:
//
//   своя Qwen2.5-VL-32B        капуста 200  мясо 150   «Мясное блюдо»
//   qwen3-vl-plus              120          150        тушёное мясо
//   qwen-vl-max                150          120        говядина
//   qwen3-vl-32b-instruct      120           80        говядина
//   qwen3-vl-flash             150          180        тушеная говядина
//   qwen3-vl-8b-instruct       без граммов   —         увидела одну капусту
//   qwen3-vl-235b-a22b-think   200          150        говядина
//   qwen3-vl-235b-a22b-instr   пусто        пусто      —
//
// Разброс между моделями меньше, чем расстояние любой из них до правды, и модель на
// 235 миллиардов с рассуждением дала ровно то же, что бесплатная 32B. Размер не
// помогает; совпадает с исследованием NIH, где коммерческие приложения занижают
// калорийность на треть, а надёжность порции 39%.
//
// Печень не узнала ни одна: пять моделей уверенно сказали «говядина». Уверенное
// неверное имя ХУЖЕ разряда: оно найдёт в базе говядину и молча подставит её КБЖУ.
//
// Что уже проверено и НЕ помогает (не пробовать снова): точка отсчёта в кадре — см.
// --scale ниже; поле с оценкой доли тарелки перед граммами; запрет называть разряд
// вместо продукта. Чинит это человек словами в описании, см. measure-merge-items.mjs.
//
...
/// Подсказка о предмете известного размера в кадре. Проверяем гипотезу: модель
/// заметила планшет, но назвала его смартфоном — а планшет вдвое длиннее, и если
/// она молча мерит кадр по телефону, всё в нём кажется мельче, чем есть.
const SCALE_HINT = arg("scale", null);
...
const PROMPT =
  ...
  (SCALE_HINT ? `\n\nВ кадре есть предмет известного размера: ${SCALE_HINT}. Меряй еду по нему.` : "");
  ...
  properties: {
    ...
    foods_on_the_photo: {
      ...
      items: {
        ...
        properties: {
          name: {
            description: "Название по-русски, 1-3 слова, и НАЗЫВАЙ ПРОДУКТ, а не разряд: «печень», " +
              "«говядина», «треска» — но не «мясное блюдо» и не «гарнир». Разряд вместо продукта " +
              "бесполезен: по нему не найти ни нутриентов, ни того же блюда в записях. Если вид " +
              "продукта по снимку не определить, скажи хотя бы то, что видно наверняка.",
            type: "string",
          },
          how_big_is_the_portion: {
            description: "ПО-РУССКИ, коротко: какую часть тарелки занимает эта еда и насколько " +
              "толстым слоем лежит. Обеденная тарелка — около двадцати семи сантиметров поперёк, " +
              "от её края и пляши. Оценивается ДО граммов, потому что глазомер обманывает в одну " +
              "сторону: порцию на снимке привычно занижают вдвое, а лежит она слоем, и объём растёт " +
              "быстрее, чем видимое пятно.",
            type: "string",
          },
          grams: {
            description: "Сколько этого на снимке, в граммах, исходя из доли тарелки и толщины слоя " +
              "из предыдущего поля. Если оценить нельзя вовсе — null.",
            ...NUM_OR_NULL,
          },
        },
        required: ["name", "how_big_is_the_portion", "grams"],
      },
    },
  ...
  for (const f of files) {
    ...
    if (a.photo_kind === "food") {
      const list = (a.foods_on_the_photo || [])
        .map((i) => `${i.name}${i.grams === null ? "" : ` ${i.grams} г`}  (${i.how_big_is_the_portion || "—"})`)
        .join("\n       ");
