// Планка по белку = доля калорийной планки, зажатая двумя границами.
//
// Правило: 30 % калорийной планки (в грамме белка 4 ккал), но не ниже 1,6 г на кг
// БЕЗЖИРОВОЙ массы (Deurenberg) и не выше 2,2 г на кг ПОЛНОГО веса. Пересчёт —
// вместе с калорийной планкой.
//
...
//
//   1800 ккал → 0.30 × 1800 / 4 = 135 г  — доля внутри границ
//   2600 ккал → 195 г, выше потолка      — 142 г
...
const CASES = [
  { kcal: 1800, want: 135, why: "доля 30 % внутри границ" },
...
// `set_calorie_goal`. Проверяем, что белок поехал следом и ровно по правилу.
{
  ...
  const want = Math.round(Math.min(Math.max(0.3 * (kcal ?? 0) / 4, 74.15), 141.9));
  const shown = await gaugeTarget(page);
