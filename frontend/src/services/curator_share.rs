...
/// ещё их заполняет.
///
/// Час перелома едет явно, хотя сейчас это полночь: куратор не обязан знать, что
/// у нас за правило, а правило может и смениться — в этой самой ветке оно уже
/// менялось с 04:00 на полночь.
///
...
fn build_clock() -> Value {
    use chrono::Duration;
    /// Час, в который у человека начинается новый день. Полночь: сутки в дневнике
    /// теперь календарные.
    const DAY_FLIP_HOUR: u32 = 0;
