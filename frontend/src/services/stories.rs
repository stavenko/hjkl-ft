...
const S9: &[Frame] = &[
    ...
    //
    // ЦЕЛИКОМ, а не `Cover`: в исходнике фигура стоит в левой трети (справа было
    // пустое поле стока, оно обрезано). `Cover` растягивает снимок на 145 % ширины
    // от центра и срезает по краям — шарик уходил за левый край экрана.
    Frame {
        bg: Bg::Dark,
        media: Media::CoverWhole("fiber-bloating.jpg"),
        accent: GREEN,
        kicker: Loc { en: "Fibre", ru: "Клетчатка" },
