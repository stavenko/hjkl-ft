...
//   * ОРГАНЫ против МЯСА — в гем входят все органы, в красное мясо только мышечные:
//     язык и сердце млекопитающего входят, печень нет;
//   * ПТИЦА против МЛЕКОПИТАЮЩИХ — птица не красное мясо, никакая;
...
const CASES = [
  ...
  // Страуса в категориях нет: строка про него была карманом для незнакомых слов
  // (в него упал «Голец»), а ест его один человек из тысячи.
  ["Страусятина", false, false, "птица, и в категориях её больше нет"],
  ...
  // Органы: в гем входят все, в красное мясо — только МЫШЕЧНЫЕ. Язык и сердце —
  // мышцы, и решением владельца они тратят недельные граммы: по составу это то же
  // мясо, с тем же гемовым железом и тем же жиром. Печень не мышца — она вне.
  ["Печень говяжья", false, false, "орган, но не мышца"],
  ["Сердечки куриные", false, false, "сердце — мышца, но птичья: не млекопитающее"],
  ["Язык говяжий", true, false, "мышца млекопитающего, хоть и орган"],
  ...
  // РЕДКИЕ ПРОМЫСЛОВЫЕ ИМЕНА — боевой случай. «Голец» модель объявила мясом
  // страуса, и съеденная рыба дала в недельной планке красного мяса пик в 1200 г.
  // Незнакомое слово тянет к любой знакомой категории, и строка про страуса
  // работала для этого карманом — поэтому её и не стало.
  ["Голец", false, false, "рыба"],
  ["Пикша", false, false, "рыба"],
  ["Сайда", false, false, "рыба"],
  ["Муксун", false, false, "рыба"],
  ["Зубатка", false, false, "рыба"],
...
// ONLY=подстрока[,подстрока] — прогнать прицельно по больным позициям. Каждый
// продукт идёт своим конвейером, соседи по набору на ответ не влияют.
if (process.env.ONLY) {
...
// Приложение печатает по каждому продукту вердикт и обоснование — без них видно
// только «мимо», но не ЧЕМ модель это объяснила. Именно обоснование показало, что
// «Голец» уходил в красное мясо как «мясо страуса».
const said = new Map();
page.on("console", (m) => {
  const g = m.text().match(/^(красное мясо|переработанное мясо) «(.+?)»: (.+)$/);
...
for (const [name, wantRed, wantProc, why] of CASES) {
  ...
  if (!okRed) console.log(`     красное: ${said.get(`красное мясо|${name}`) ?? "(в консоль ничего не пришло)"}`);
  if (!okProc) console.log(`     переработанное: ${said.get(`переработанное мясо|${name}`) ?? "(в консоль ничего не пришло)"}`);
}
