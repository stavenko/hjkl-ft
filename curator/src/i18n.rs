...
//!
//! Слов для экрана установки здесь НЕТ и быть не должно: шаги рисует общий крейт
//! `pwa-prompt`, и слова живут там же, рядом со своей разметкой. Здесь остаётся
//! только то, что у кураторского приложения своё, — заголовок этого экрана и
//! объяснение, зачем ставить.
...
fn ru(key: &str) -> &'static str {
    match key {
        ...
        "client.request_what" => "Что запросить",
        "client.request_new" => "Только новое",
        "client.request_new_hint" => "Всё, что клиент накопил после прошлого отчёта.",
        "client.request_all" => "Все данные",
        "client.requested" => "Запрос отправлен",
        "client.waiting" => "Ждём ответа с {date}",
        "client.report_at" => "Отчёт от {date}",
        "client.chat" => "Открыть чат",
        ...
        "settings.push_on" => "Включены: сообщения клиентов приходят, даже когда приложение закрыто.",
        "settings.push_off" => "Выключены. Без них сообщение клиента вы увидите, только открыв приложение.",
        "settings.push_enable" => "Включить уведомления",
        "settings.push_again" => "Подписаться заново",
        "settings.push_failed" => "Не удалось. Если запретили уведомления браузеру — разрешите их в настройках телефона и попробуйте снова.",
        "settings.push_unsupported" => "Уведомления работают только в установленном приложении. Поставьте его на домашний экран и откройте с иконки.",
...
fn en(key: &str) -> &'static str {
    match key {
        ...
        "client.request_what" => "What to ask for",
        "client.request_new" => "Only what is new",
        "client.request_new_hint" => "Everything since their previous report.",
        "client.request_all" => "Everything",
        "client.requested" => "Request sent",
        "client.waiting" => "Waiting since {date}",
        "client.report_at" => "Report from {date}",
        "client.chat" => "Open chat",
        ...
        "settings.push_on" => "On: client messages arrive even when the app is closed.",
        "settings.push_off" => "Off. Without them you will only see a client's message by opening the app.",
        "settings.push_enable" => "Turn on notifications",
        "settings.push_again" => "Subscribe again",
        "settings.push_failed" => "Did not work. If you blocked notifications for the browser, allow them in your phone settings and try again.",
        "settings.push_unsupported" => "Notifications only work in the installed app. Add it to your home screen and open it from there.",
        "settings.save" => "Save",
...
mod tests {
    ...
    fn tablicy_soglasovany() {
        for key in [
            ...
            "client.invite_body", "client.no_report", "client.request", "client.request_what", "client.request_new",
            "client.request_new_hint", "client.request_all",
            ...
            "settings.push", "settings.push_on", "settings.push_off", "settings.push_enable",
            "settings.push_again", "settings.push_failed", "settings.push_unsupported",
