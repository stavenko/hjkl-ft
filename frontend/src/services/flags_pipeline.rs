...
//! вопросов.
//!
//! ПЕРЕД ПРАВКОЙ ПРОМПТОВ — прочитайте `docs/food-identity.md`. Там собраны отказы,
//! которые мы уже разобрали (рифма «огурец» → «голец», марки, схема, уезжающая
//! модели текстом), и порядок замера. Документ живёт в `docs/`, а не здесь,
//! намеренно: всё, что попадает в этот файл СТРОКАМИ, уезжает модели в промпт.
//!
