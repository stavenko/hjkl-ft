...
**Проверка.** Две, и они про разное:

- `node scripts/curator-e2e.mjs` — договор ВОРКЕРА: весь путь по его ручкам, без браузера.
- `node scripts/check-curator-flow.mjs` — договор ПРИЛОЖЕНИЯ, четыре утверждения:
  без куратора приложение ведёт планки само (калории, шаги, белок); привязка проходит
  ПО ССЫЛКЕ через экран согласия; с куратором ни одна планка не двигается; передача
  работает в обе стороны (директива и запрос доезжают до человека, отчёт — до
  куратора, последним днём вчерашним). Посев тот же, что у
  `check-calorie-planka-weekly.mjs`, — отличается только наличием куратора, поэтому
  разница в поведении и есть доказательство.

  Девять постоянных планок туда намеренно не входят: они чистые функции от профиля и
  калорийной планки, и их считает `cargo test -p plankas`. Браузер нужен там, где
  правило зависит от ЗАПУЩЕННОГО приложения.

Браузерная проверка поднимает собранный `dist` на 127.0.0.1 и проксирует обращения к
живым dev-воркерам (`scripts/lib/devserver.mjs`): в части окружений браузер наружу не
ходит, а node ходит. Вход обходится минтом токена тем же секретом, что у dev-воркера, —
паскей проверяет ЛИЧНОСТЬ, а проверкам нужно всё, что происходит после входа.
