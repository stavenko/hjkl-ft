...
//!
//! Директива несёт только данные: какой вид планки и какое число. Сам текст — и
//! системная плашка в чате, и письмо в почтовый ящик — собирается здесь, из строк
//! приложения. Иначе язык переписки зависел бы от настроек куратора: он выставил
//! себе английский, а человек с русским приложением получил бы английскую плашку.
//!
//! Письмо приходит на КАЖДУЮ правку планки, а не только на ту, что человек увидел
//! в чате: планка меняет то, по чему его судят индикаторы, и узнать об этом он
//! обязан, даже не открывая переписку.
//!
...
/// Сколько знаков после запятой осмысленно у этой планки — из общего крейта, где
/// про каждый вид известно всё остальное.
fn decimals(key: &str) -> usize {
    plankas::Kind::from_key(key).map(|k| k.decimals()).unwrap_or(0)
}
...
/// Перечень ДЕЙСТВУЮЩИХ планок с числами — для письма об отвязке.
///
/// Человеку, которого перестал вести куратор, мало сказать «планки остались как
/// есть»: часть из них вернулась к нашим правилам, часть осталась кураторской, и
/// какие именно числа теперь соблюдать — из этой фразы не следует. Поэтому здесь
/// они названы поимённо.
///
/// Вид, у которого планки нет вовсе (шаги до открытия темы), пропускается: строка
/// «Шаги — не задано» ничего не сообщает.
pub fn planka_list() -> String {
