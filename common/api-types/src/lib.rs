...
pub struct Food {
    ...
    pub archived: bool,
    /// УСТАРЕЛО. Ресторанная еда: не идёт в состав рецептов, несёт надбавку +20% к
    /// калориям и рисуется в списках пунктиром.
    ///
    /// Галочку убрали из интерфейса вместе с «не съел целиком»: она стояла в окне
    /// веса, ставилась редко, а надбавка в 20% была догадкой, за которую человек не
    /// отвечал. Поле ОСТАЁТСЯ и по-прежнему учитывается: продукты с этой пометкой у
    /// людей есть, и менять их калорийность задним числом нельзя. Новые продукты
    /// получают `false`.
    ///
    /// Убрать поле совсем можно будет вместе с миграцией, которая впишет надбавку в
    /// сами калории продукта; пока такой миграции нет.
    #[serde(default)]
...
pub struct DiaryEntry {
    ...
    /// позиции `items` свои).
    pub grams: f64,
    /// УСТАРЕЛО. Несъеденное (кости, косточки, кожура) в граммах; калории считались
    /// по `grams - waste_grams`.
    ///
    /// Спрашивать это перестали: галочка стояла в окне веса и почти никем не
    /// заполнялась, а место занимала у каждого продукта. Поле ОСТАЁТСЯ и
    /// по-прежнему учитывается в расчётах — у прежних записей значения есть, и
    /// обнулять чужие числа задним числом нельзя. Новые записи получают ноль.
    ///
    /// Убрать поле совсем можно будет вместе с миграцией, которая сложит
    /// `grams - waste_grams` в `grams`; пока такой миграции нет.
    #[serde(default)]
    pub waste_grams: f64,
    ...
    /// Почему разобрать НЕ удалось — словами, для человека.
    ///
    /// Молча подставлять справочные значения вместо непрочитанной этикетки нельзя:
    /// человек об этом не узнает (§6.6). Поэтому неудача не прячется в консоль, а
    /// показывается в строке дневника и в форме правки.
    ///
    /// Пусто у всех записей, заведённых до этого поля, и это верно: у них неудачи
    /// не было, была тишина.
    #[serde(default)]
    pub recognition_error: Option<String>,
    /// Сколько раз разбор уже проваливался.
    ///
    /// Без счётчика безнадёжная запись перебиралась бы очередью при каждом запуске
    /// приложения и жгла бы запросы к модели вечно. После нескольких попыток запись
    /// оставляют в покое: человек либо поправит снимки и описание — и тогда счётчик
