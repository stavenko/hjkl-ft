...
fn ru(key: &str) -> &'static str {
    match key {
        ...
        // Запасные пути входа
        "login.alt_key" => "Войти ключом",
        "login.alt_code" => "Код в Telegram",
        "login.alt_phrase" => "Фразой восстановления",
        "login.code_sub" => "Пришлём одноразовый код в Telegram — туда же, куда приходят письма о подписке.",
        "login.code_send" => "Прислать код",
        "login.code_sending" => "Отправляем…",
        "login.code_verify" => "Войти",
        "login.code_invalid" => "Код не подошёл. Проверьте и введите заново.",
        "login.code_too_often" => "Код уже отправляли. Подождите минуту и попробуйте снова.",
        "login.code_offer" => "Ключ не сработал. Подписка на этом аккаунте активна — можем прислать код в Telegram.",
        "login.code_goto" => "Прислать код в Telegram",
        "login.no_account_sub" => "",
        "login.no_account" => "Ключ не сработал, а подписки на этом аккаунте нет. Оформите её в приложении питания или у нашего бота — после этого вход сюда откроется.",
        "login.phrase_sub" => "Пять слов, которые вы записали в настройках — здесь или в приложении питания.",
        "login.phrase_placeholder" => "пять слов через пробел",
        "login.phrase_enter" => "Войти по фразе",
        "login.phrase_invalid" => "Такая фраза не подошла. Проверьте порядок и написание слов.",
        "login.phrase_too_often" => "Слишком много попыток. Подождите и попробуйте снова.",
        "login.phrase_failed" => "Не удалось войти по фразе.",
...
fn en(key: &str) -> &'static str {
    match key {
        ...
        "login.alt_key" => "Sign in with your key",
        "login.alt_code" => "Code in Telegram",
        "login.alt_phrase" => "Recovery phrase",
        "login.code_sub" => "We'll send a one-time code to Telegram — the same place your subscription letters arrive.",
        "login.code_send" => "Send the code",
        "login.code_sending" => "Sending…",
        "login.code_verify" => "Sign in",
        "login.code_invalid" => "That code didn't work. Check it and try again.",
        "login.code_too_often" => "A code was just sent. Wait a minute and try again.",
        "login.code_offer" => "The key didn't work. This account has an active subscription — we can send a code to Telegram.",
        "login.code_goto" => "Send a code to Telegram",
        "login.no_account_sub" => "",
        "login.no_account" => "The key didn't work, and this account has no subscription. Get one in the nutrition app or via our bot — access here opens right after.",
        "login.phrase_sub" => "The five words you saved in settings — here or in the nutrition app.",
        "login.phrase_placeholder" => "five words separated by spaces",
        "login.phrase_enter" => "Sign in with the phrase",
        "login.phrase_invalid" => "That phrase didn't match. Check the order and spelling.",
        "login.phrase_too_often" => "Too many attempts. Wait a little and try again.",
        "login.phrase_failed" => "Couldn't sign in with the phrase.",
