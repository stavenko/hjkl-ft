//! Вход в приложение тренировок паскеем.
//!
//! Тот же auth-worker, что у остальных приложений, но область ключей здесь
//! НАМЕРЕННО общая с приложением худеющего, а не своя, как у админки и куратора:
//! требование продукта — входить «тем же ключом, что и на fit.renorma.app». В
//! проде это обеспечивает rp_id `renorma.app` (registrable suffix обоих
//! поддоменов), см. `GYM_RP_ID` в `cloudflare/auth-worker/wrangler.toml`.
//!
//! Отсюда и порядок на экране входа: главное действие — ВОЙТИ существующим
//! ключом, а не завести новый. Обратное было бы ловушкой: человек, у которого
//! ключ уже есть, завёл бы вторым нажатием второй аккаунт — пустой и без
//! подписки.
//!
//! На dev-стенде область СВОЯ и общей быть не может: `pages.dev` — публичный
//! суффикс, и один rp_id два таких поддомена не покрывает. Там ключ придётся
//! завести отдельный — это ограничение стенда, а не продукта.
//!
//! Разметка WebAuthn перенесена из кураторского приложения без изменений;
//! сессия — `user_id` и `auth_token` в localStorage, ни базы, ни синка.
...
const KEY_USER_ID: &str = "gym_user_id";
const KEY_AUTH_TOKEN: &str = "gym_auth_token";
...
// ── Запросы ОТ ИМЕНИ сессии ─────────────────────────────────────────────────
// Отдельно от `post_json`: ручки `/add-device/*` и `/recovery/phrase*` закрыты
// токеном, а origin церемонии сервер берёт из заголовка сам.

async fn post_json_auth(path: &str, body: &serde_json::Value) -> Result<serde_json::Value, String> {
    request_auth("POST", path, Some(body)).await
}

async fn get_json_auth(path: &str) -> Result<serde_json::Value, String> {
    request_auth("GET", path, None).await
}
...
async fn request_auth(
    ...
    let token = get_token().ok_or_else(|| "нет сессии".to_string())?;
    let url = format!("{}{}", auth_base_url(), path);
...
/// Завести ещё один паскей для УЖЕ вошедшего аккаунта — на этом устройстве.
///
/// Нужно там, где ключ с другого устройства не подходит: на iOS у установленного
/// приложения своё хранилище, а на dev-стенде у зала своя область ключей вовсе.
/// Повторяет `register()`, но ходит в закрытые токеном `/add-device/*` — origin
/// церемонии сервер берёт из заголовка, а не с наших слов.
pub async fn add_passkey(display_name: &str) -> Result<(), String> {
    let fingerprint = generate_fingerprint();
    ...
    let begin_resp = post_json_auth("/add-device/begin", &begin_body).await.map_err(|e| {
        leptos::logging::error!("add-device/begin failed: {e}");
    ...
    post_json_auth(
        "/add-device/finish",
        &serde_json::json!({ "user_id": user_id, "credential": credential_json }),
    )
    .await
    .map_err(|e| {
        leptos::logging::error!("add-device/finish failed: {e}");
        format!("{ERR_NETWORK}: {e}")
    })?;
    Ok(())
}
...
//
// Фраза принадлежит АККАУНТУ, а не приложению: она та же самая, что в приложении
// питания, и заведённая там показывается здесь как есть. Отсюда и предупреждение
// в интерфейсе — новая фраза заменяет старую всюду.

/// Текущая фраза аккаунта открытым текстом, или None, если её не заводили.
pub async fn get_backup_phrase() -> Result<Option<String>, String> {
    let resp = get_json_auth("/recovery/phrase").await?;
    Ok(resp.get("phrase").and_then(|v| v.as_str()).map(|s| s.to_string()))
}

/// Записать фразу. Возвращает статус сервера: `"ok"` | `"taken"` (совпала с
/// чужой — сгенерировать другую) | `"too_short"`.
...
/// Завести НОВЫЙ ключ (и новый аккаунт) на этом устройстве. Возвращает `sub`.
///
/// Путь второстепенный и спрятан за ссылкой: у пришедшего из приложения питания
/// ключ уже есть, и заводить второй ему не надо. Он здесь для того, у кого
/// аккаунта нет вовсе, — но подписки у такого аккаунта тоже нет, и дальше он
/// упрётся в экран «нужна подписка». Это честнее, чем не пустить вовсе.
...
/// Войти СУЩЕСТВУЮЩИМ паскеем (discoverable credential) — главный путь входа.
///
/// В проде браузер предложит здесь ровно те ключи, что заведены на
/// `renorma.app`, то есть в том числе созданный в приложении питания.
