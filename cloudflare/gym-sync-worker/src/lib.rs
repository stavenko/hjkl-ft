// Синхронизация данных ПРИЛОЖЕНИЯ ТРЕНИРОВОК: per-user journaled batch log.
//
// Тот же протокол и тот же код, что у `sync-worker`, но СВОЙ DO-неймспейс: у
// приложения тренировок своя история, и она не должна проигрываться клиентам
// приложения худеющего (и наоборот) — журнал читается целиком, а незнакомый
// `store` у соседа уходит в лог ошибкой. Токен общий: `sub` тот же user_id.
//
...
fn is_allowed_origin(origin: &str) -> bool {
    // Свой dev-домен — первым: сюда ходит именно приложение тренировок. Fit-dev
    // оставлен намеренно, чтобы локальная отладка через знакомый стенд работала.
    if origin == "https://renorma-gym-dev.pages.dev" || origin == "https://renorma-fit-dev.pages.dev" {
        return true;
    }
...
const INTERNAL_HOST: &str = "gym-sync-worker";
...
async fn handle_internal_wipe(req: &Request, env: &Env) -> Result<Response> {
    ...
    console_log!("gym-sync: wiped journal for {user_id}");
