...
const CASES = [
  ...
  // Редис — КОРНЕПЛОД, а корнеплоды в «овощи и фрукты» не идут (то же правило, по
  // которому туда не попадают картофель и сельдерейный корень). Здесь сначала
  // стояло «да» — ошибка эталона, конвейер отвечал верно: «корень, клетчатка 1.8 г».
  ["Редиска", false, false, false, false, false],
