...
async fn calorie_hint_text(planka: Option<f64>) -> String {
    ...
    let why = if f > 1.001 {
        ...
        "а вес снижается заметно медленнее комфортного (или не снижается вовсе) — мы снизили \
         планку на 5%, чтобы создать мягкий дефицит"
    } else {
        "а вес снижается комфортно — или разброс взвешиваний пока не даёт отличить темп от \
         комфортного, и тогда мы тем более не трогаем планку"
    };
