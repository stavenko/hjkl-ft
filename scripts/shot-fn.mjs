// Screenshot Story 4 frame 3 (dairy Cover layout), waiting for the cover image to
// actually decode. Seeding mirrors the reliable open-story4-frame6.mjs (flags + profile).
import { chromium } from "playwright";
const FE = process.argv[2];
const TAPS=Number(process.argv[3]||0); const OUT=process.argv[4]||"frame"; const uid = "s4g-" + Date.now();
...
for (let i = 0; i < TAPS; i++) { await page.mouse.click(300, 700); await page.waitForTimeout(550); }
await page.waitForFunction(() => {
  const im = [...document.querySelectorAll('img')].find(i => /calcium-(bones|plants|fish|dairy)/.test(i.src));
  return im && im.complete && im.naturalWidth > 0;
}, { timeout: 8000 }).catch(() => console.log("img wait timed out"));
