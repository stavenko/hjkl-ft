...
// 1st request → client shares it.
r = await fetch(`${SUP}/conversations/${clientSub}/reply`, { method: "POST", headers: { Authorization: `Bearer ${expertTok}`, "Content-Type": "application/json" }, body: JSON.stringify({ client_id: "req1-" + ts, text: "Запрос 1", kind: "data_request", payload: JSON.stringify({ dataset: "food" }) }) });
...
console.log("1st request: shared ✓");

// 2nd request for the SAME dataset (curator wants fresh data).
r = await fetch(`${SUP}/conversations/${clientSub}/reply`, { method: "POST", headers: { Authorization: `Bearer ${expertTok}`, "Content-Type": "application/json" }, body: JSON.stringify({ client_id: "req2-" + ts, text: "Запрос 2", kind: "data_request", payload: JSON.stringify({ dataset: "food" }) }) });
await cpage.waitForTimeout(4000); // let it poll in
// Reload the chat (fresh render), then inspect the LAST request panel's state.
await cpage.reload({ waitUntil: "domcontentloaded" });
await cpage.waitForTimeout(3000);
const panels = cpage.getByTestId("live-request");
const n = await panels.count();
const shareButtons = await cpage.getByTestId("live-request-share").count();
const doneMarks = await cpage.getByTestId("live-request-done").count();
await cpage.screenshot({ path: "reshare-client.png", fullPage: true });
console.log(`request panels: ${n} | share buttons: ${shareButtons} | done marks: ${doneMarks}`);

await cctx.close(); await b.close();
// The 2nd request MUST be re-fulfillable → at least one share button present.
const ok = n >= 2 && shareButtons >= 1;
console.log(ok ? "\n✅ second request offers a share button" : "\n❌ REPRO — a repeat request shows «уже отправлено», no button to re-share");
process.exit(ok ? 0 : 1);
