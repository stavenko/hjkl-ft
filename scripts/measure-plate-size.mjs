...
//
//   по рамкам объектов             18%   ← и +2% там, где эталон виден целиком
//   просто спросить                16%
...
//
// Видна одна причина провалов: модель не МЕРИТ по кадру, а ВСПОМИНАЕТ типичное. Дать
// ей перечисление — выберет самое ходовое. Попросить отношение — назовёт привычное
// 1.8, хотя на рыбе тарелка длиннее вилки всего в 1.08 раза.
//
// Рамки — исключение, и потому что это ДРУГОЕ умение: не прикидка размера, а
// локализация, которой эти модели учены отдельно. Модель показывает, ГДЕ посуда и
// где предмет, а отношение считает код. На рыбе, где вилка видна целиком и опознана
// верно, вышло 19.2 см при правде 19. На печени модель взяла планшет, назвала его
// телефоном и обвела лишь видимую часть — остальное закрыто тарелкой, укороченная
// рамка эталона и завысила ответ до 28.3.
//
// Значит упирается не в модель, а в то, есть ли в кадре ЦЕЛИКОМ ВИДНЫЙ предмет
// точного размера. Пластиковая карта (85.6 x 54 мм по стандарту) даёт и то и другое,
// и лежит с посудой в одной плоскости — при съёмке под углом обе искажаются одинаково
// и отношение сохраняется.
//
...
/// Не прикидка, а ЛОКАЛИЗАЦИЯ: модель показывает, где на кадре посуда и где предмет
/// известного размера, а отношение считает код. Модель не умеет прикидывать размеры,
/// но показывать рамки объектов её учили специально — и это разные умения.
const BBOX = {
  type: "object",
  properties: {
    reference: {
      description: "Предмет известного размера в кадре: \"fork\" — столовая вилка, \"spoon\" — " +
        "ложка, \"knife\" — нож, \"phone\" — телефон. Ничего такого нет — \"none\".",
      type: "string",
      enum: Object.keys(REF_CM),
    },
    plate_box: {
      description: "Рамка ПОСУДЫ в долях кадра: [x0, y0, x1, y1], где 0 — левый верхний угол, " +
        "1 — правый нижний. Рамка охватывает тарелку целиком, по внешнему краю.",
      type: "array", items: { type: "number" },
    },
    reference_box: {
      description: "Рамка предмета из поля reference, в тех же долях кадра: [x0, y0, x1, y1]. " +
        "Охватывает предмет целиком, от края до края. Предмета нет — [0,0,0,0].",
      type: "array", items: { type: "number" },
    },
  },
  required: ["reference", "plate_box", "reference_box"],
};

/// Из рамок — сантиметры. У посуды берём большую сторону рамки: снятая под углом
/// тарелка становится эллипсом, и длинная ось ближе к настоящему поперечнику. У
/// вытянутого предмета — диагональ рамки: вилка лежит наискось, и её длина это
/// диагональ, а не сторона.
///
/// Доли кадра переводим в ПИКСЕЛИ прежде, чем мерить длины. Кадр не квадратный
/// (1536x1152), и одна и та же доля по горизонтали и по вертикали — разное число
/// пикселей; считать длину прямо в долях значит растянуть кадр в квадрат и получить
/// вздор. На этом я один раз уже ошибся и чуть не записал промах себе в заслугу
/// модели.
function fromBoxes(a, W, H) {
...
const WAYS = {
  ...
  "по рамкам объектов": { schema: BBOX, get: fromBoxes, needsSize: true },
...
/// Размеры JPEG из его собственных заголовков — чтобы не тащить сюда зависимость
/// ради двух чисел.
function jpegSize(buf) {
  ...
  while (i < buf.length) {
    ...
    if (marker >= 0xc0 && marker <= 0xcf && marker !== 0xc4 && marker !== 0xc8 && marker !== 0xcc) {
      return [buf.readUInt16BE(i + 7), buf.readUInt16BE(i + 5)];  // ширина, высота
    }
...
async function main() {
  ...
  for (const [way, { schema, get, needsSize }] of Object.entries(WAYS)) {
    if (ONE_WAY && way !== ONE_WAY) continue;
    console.log(`— ${way}`);
    let worstArea = 0;
    for (const c of CASES) {
      const buf = readFileSync(`scripts/fixtures/${c.file}`);
      const image = buf.toString("base64");
      const got = [];
      for (let i = 0; i < N; i++) {
        try {
          const ans = await ask(token, image, schema);
          got.push(needsSize ? get(ans, ...jpegSize(buf)) : get(ans));
        }
