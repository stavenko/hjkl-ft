// «Мигающие» GIF-подсказки для экранов установки PWA на iPhone — Chrome и Safari.
// Тот же росчерковый бурст, что у Яндекса (scripts/shot-ios-yandex-gifs.mjs) и на
// dashboard-*.gif, поверх присланных скриншотов живого интерфейса:
//   Chrome: 1) адресная строка — значок «Поделиться» в правом краю
//           2) меню Chrome — строка «На экран „Домой“»
//   Safari: 1) нижняя панель — значок «Поделиться» посередине
//           2) системный лист — строка «На экран „Домой“»
// Третий шаг у обоих — системный диалог с кнопкой «Добавить»; он один и тот же на
// весь iOS, снят в яндексовском прогоне (ios-ya-add.gif) и переиспользуется.
import { chromium } from "playwright";
import { execSync } from "node:child_process";
import { mkdirSync, rmSync, readFileSync } from "node:fs";

const DIR = process.argv[2] ||
  "/private/tmp/claude-501/-Users-vasilijstavenko-projects-hjkl-ft/56df53af-a1ed-4117-8e82-8a1f8aad90e8/scratchpad/ios-chrome";
...
const drawSpark = ({ rects, frame }) => {
  ...
  for (const rect of rects) {
    ...
    // Широкие цели (строка меню) излучают росчерки из периметра скруглённого
    // прямоугольника, квадратные (значок) — радиально из центра.
...
// то есть половина от присланных ретиновых кадров: 1125 → 562).

// 1) Адресная строка Chrome: значок «Поделиться» у правого края.
await makeGif({ img: "bar.jpg", w: 562, h: 95, out: "ios-chrome-share", pad: 10,
  rects: [{ x: 496, y: 33, w: 32, h: 32 }] });

// 2) Меню Chrome: строка «На экран „Домой“».
await makeGif({ img: "menu.jpg", w: 562, h: 330, out: "ios-chrome-add", pad: 10,
  rects: [{ x: 20, y: 178, w: 520, h: 46 }] });

// 3) Нижняя панель Safari: значок «Поделиться» — посередине ряда кнопок, а не в
//    адресной строке, как у Chrome.
await makeGif({ img: "safari-bar.jpg", w: 562, h: 145, out: "ios-safari-share", pad: 10,
  rects: [{ x: 265, y: 98, w: 32, h: 32 }] });

// 4) Системный лист iOS: строка «На экран „Домой“».
await makeGif({ img: "safari-sheet.jpg", w: 562, h: 330, out: "ios-safari-home", pad: 10,
  rects: [{ x: 24, y: 160, w: 515, h: 58 }] });
