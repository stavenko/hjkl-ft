...
- `node scripts/check-curator-planka-freeze.mjs` — договор ПРИЛОЖЕНИЯ: недельный цикл
  считает планку, когда куратора нет, и молчит, когда есть; директива куратора и его
  запрос доезжают до приложения худеющего и применяются в нём. Тот же посев, что у
