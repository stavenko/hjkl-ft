...
async fn calorie_hint_text(planka: Option<f64>) -> String {
    ...
    // говорит о положении веса относительно ЦЕЛЕВОЙ полосы, а не о снижении:
    // «уходит вниз сильнее нужного» верно и для похудения, и для набора, а
    // «снижается слишком быстро» человеку на наборе — бессмыслица.
    let f = local::planka_factor(&trend, weight_kg, profile::planka_goal());
    let why = if f > 1.001 {
        "а вес уходит вниз сильнее, чем нужно, — мы приподняли планку на 5%"
    } else if f < 0.999 {
        "а вес уходит вверх сильнее, чем нужно, — мы снизили планку на 5%"
    } else {
        "а вес идёт так, как задумано, — или разброс взвешиваний пока не даёт отличить его \
         от задуманного, и тогда мы тем более не трогаем планку"
    };
    ...
    format!(
        "Поскольку ваша цель — {goal}, {why}. Вес мы смотрим за последние {days} дней — \
...
/// Пояснения «?» для двух мясных ограничений.
fn red_meat_hint_text() -> String {
    use crate::services::red_meat;
    format!(
        "Красное мясо — мышцы млекопитающих: говядина, свинина, баранина, козлятина, \
         дичь. Связь с раком кишечника растёт с количеством, поэтому здесь не цель, а \
         планка: не больше {:.0} г сырого мяса в неделю.\n\n\
         Считаем по белку и переводим в сырой вес: при готовке мясо теряет до трети \
         веса, и {:.0} г тушёного — это заметно больше {:.0} г сырого. В блюдах \
...
fn egg_hint_text() -> String {
    use crate::services::egg;
    format!(
        "Яйцо — эталон белка: по усвояемости и составу аминокислот оно превосходит мясо, \
         рыбу и молочное. Одно яйцо закрывает треть суточной нормы холина, 28 % селена, \
         19 % B12 и 16 % йода, а плотных источников холина в обычном рационе единицы.\n\n\
         Планка здесь МИНИМУМ, а не потолок: {:.0} яиц в неделю надёжно закрывают холин и \
         селен и занимают всего 8 % вашего лимита насыщенных жиров.\n\n\
...
/// Клетчатка. Норма привязана к КАЛОРИЙНОСТИ, поэтому в тексте стоит своё число:
/// у человека на 2000 ккал и на 3000 ккал она разная, и общая фраза «ешьте больше
...
fn fiber_hint_text(planka: Option<f64>) -> String {
    ...
    format!(
        "Клетчатка — то, что мы сами не перевариваем, а отдаём кишечным бактериям. От неё          зависят стул, чувство сытости, уровень холестерина и сахара после еды.
         ...
         Норма считается от калорийности: {:.0} г на 1000 ккал, но не меньше {:.0} г в сутки.          При вашей планке это {daily:.0} г в день, то есть {:.0} г за неделю.

         Индикатор НЕДЕЛЬНЫЙ, и это не мелочь: клетчатка спокойно терпит неравномерность —          важна сумма за неделю, а не то, что вышло в отдельный день. Шкалы у неё поэтому нет.

         Считается из состава еды: граммы клетчатки берутся из разбора каждого продукта,          отдельно отмечать ничего не нужно.",
        fiber::G_PER_1000_KCAL,
        fiber::MIN_G_PER_DAY,
        daily * 7.0,
    )
}

fn processed_meat_hint_text() -> String {
    "Мясо глубокой переработки — то, что консервировали ради хранения: колбасы, сосиски, \
     ветчина, бекон, salami, вяленое и копчёное мясо.\n\n\
     Здесь меряется не количество, а ЧАСТОТА: связь с раком кишечника у таких продуктов \
     начинается с небольших порций, поэтому важно, как часто они на столе, а не сколько \
     их за раз.\n\n\
     Зелёный — за неделю ни одного такого дня. Один-три дня — оранжевый, четыре и больше — \
     красный. Запрещать их незачем: доза отличает лекарство от яда."
        .to_string()
}
...
fn weekly_metric_name(key: &str) -> Option<&'static str> {
    match key {
        ...
        // Единственный недельный индикатор про ОГРАНИЧЕНИЕ: закрытая неделя здесь
        // значит «уложился», а не «набрал».
        "red_meat" => Some("недельную планку красного мяса"),
        // Обратно мясу: здесь закрытая неделя значит «набрал».
        "egg" => Some("недельную планку по яйцам"),
        "fiber" => Some("недельную планку клетчатки"),
        _ => None,
    }
}
...
fn indicator_verdict(key: &str, state: IndicatorState, missed: u32) -> String {
    ...
    if let Some(what) = weekly_metric_name(key) {
        // Цвет — про ДВЕ ПОСЛЕДНИЕ недели, а не про долю за всё окно: см.
        // `indicators::weekly_state`. Восемь клеток истории ниже — отдельный
        // разговор, и `missed` относится именно к ним.
        return match state {
            Green => format!(
                "Зелёный: последнюю завершённую неделю вы закрыли {what}. Текущая неделя не \
                 судится — это набирается неделей, а не за день."
            ),
            Orange => format!(
                "Оранжевый: последняя завершённая неделя осталась незакрытой. Повторится \
                 второй раз подряд — станет красным. Всего незакрытых за восемь недель: {missed}."
            ),
            Red => format!(
                "Красный: две последние завершённые недели подряд остались незакрытыми. \
                 Закроете ближайшую — вернётся зелёный. Всего незакрытых за восемь недель: {missed}."
            ),
            Unknown => "Пока не завершилась ни одна неделя — оценивать нечего.".to_string(),
        };
    }
    // Переработанное мясо судится ЧАСТОТОЙ: промах — день, в который человек его ел.
    // Формулировать это «нормой» нельзя, нормы тут нет и быть не может.
    if key == "processed_meat" {
        return match state {
            Green => "Зелёный: за последнюю неделю вы ни разу не ели мясо глубокой \
                      переработки."
                .to_string(),
            Orange => format!(
                "Оранжевый: за последнюю неделю такие продукты были у вас {missed} дней из 7 \
                 (1–3 дня → оранжевый)."
            ),
            Red => format!(
                "Красный: за последнюю неделю такие продукты были у вас {missed} дней из 7 \
                 (4 и более → красный)."
            ),
            Unknown => "Пока недостаточно записей, чтобы оценить.".to_string(),
        };
    }
...
async fn indicator_body(key: &str, planka: Option<f64>) -> String {
    match key {
        ...
        "red_meat" => red_meat_hint_text(),
        "egg" => egg_hint_text(),
        "fiber" => fiber_hint_text(planka),
        "processed_meat" => processed_meat_hint_text(),
                                ...
                                view! {
                                    <div attr:data-ind-panel=s.key
                                        ...
                                        <DayBars series=Signal::derive(move || days.clone())
                                            ...
                                            // ЦВЕТ СТОЛБИКА — НЕ ЦВЕТ ИНДИКАТОРА.
                                            // У столбика вопрос двоичный: неделя
                                            // (или день) закрыта или нет. Пока
                                            // сюда шёл цвет состояния, незакрытые
                                            // недели красились в цвет значка — и
                                            // у зелёного индикатора провал с 1.2
                                            // порции гема выглядел как успех.
                                            miss_color=crate::components::day_bars::BAR_MISS.to_string()
