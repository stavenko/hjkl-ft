...
/// Применить кураторские директивы `set_planka_v2` — по одной на индикатор.
///
/// Директива несёт ЧИСЛО и ключ вида, больше ничего: замков нет, «вернуть нашу»
/// нет. Всё, что она делает, — пишет планку в историю, единственное место, где
/// планка живёт. Текст письма собирается ЗДЕСЬ, на языке человека: куратор мог
/// писать на своём.
///
/// Пределы проверяются перед применением. Число приходит из чужого приложения, и
/// лишний ноль не должен становиться планкой, от которой посчитается ещё и
/// клетчатка.
async fn apply_curator_planka_directives(msgs: &[LiveMessage]) {
    use crate::services::{directives, letters, plankas};

    let last = crate::services::app_flags::get(CURATOR_PLANKA_SEQ_KEY)
        .and_then(|s| s.parse::<u64>().ok())
        .unwrap_or(0);
    let mut ordered: Vec<(u64, plankas::Kind, f64)> = Vec::new();
    for m in msgs {
        ...
        let Some(kind) = v
            .get("key")
            .and_then(|k| k.as_str())
            .and_then(plankas::Kind::from_key)
        else {
            leptos::logging::warn!("директива планки: неизвестный вид в {payload}");
            continue;
        };
        let Some(amount) = v.get("amount").and_then(|a| a.as_f64()) else {
            leptos::logging::error!("директива планки {}: без числа", kind.key());
            continue;
        };
        if !kind.accepts(amount) {
            ...
            leptos::logging::error!(
                "директива планки {}: значение {amount} вне [{lo}, {hi}]",
                kind.key()
            );
            continue;
        }
        ordered.push((m.seq, kind, amount));
    }
    if ordered.is_empty() {
        return;
    }
    ...
    for (seq, kind, amount) in ordered {
        plankas::set(kind, amount).await;
        ...
        letters::add(letters::Letter {
            ...
            created_at: chrono::Local::now().to_rfc3339(),
            body: directives::set_planka_letter(kind.key(), amount),
            read: false,
            action: None,
            action_done: false,
        });
...
/// Отвечал ли сервер на этом устройстве хоть раз, то есть ЗНАЕМ ли мы адресата.
///
/// Различие между «адресат — админ» и «адресата ещё не спрашивали» существенно
/// ровно в одном месте — в недельном пересчёте планок. Флаг device-local: на
/// новом устройстве, сразу после переустановки или входа с другого телефона его
/// нет, и `current_peer()` честно отвечает «админ», потому что больше ему
/// ответить нечем.
///
/// Пересчёт при этом успевал сработать РАНЬШЕ первого опроса и двигал планку у
/// человека, которого ведёт куратор. Поймано браузерной проверкой
/// `check-curator-planka-freeze.mjs`: привязка на сервере есть, а планка
/// пересчиталась.
pub fn peer_known() -> bool {
    crate::services::app_flags::get(CURRENT_PEER_FLAG).is_some()
}

fn set_current_peer(peer: &str) {
    // Сравнение с ХРАНИМЫМ значением, а не с `current_peer()`: тот подставляет
    // «админ» по умолчанию, и ответ «админ» на пустом флаге выглядел как «ничего
    // не изменилось» — флаг не появлялся никогда. Пока это было косметикой, но с
    // тех пор как по нему различают «адресат неизвестен», такой флаг остановил бы
    // недельный пересчёт навсегда у всех, у кого куратора нет.
    if crate::services::app_flags::get(CURRENT_PEER_FLAG).as_deref() != Some(peer) {
