        ...
        properties: {
          ...
          where_grams_came_from: {
            description: "Откуда берутся граммы — решается ДО самих граммов, по строгому старшинству. " +
              ...
              "если на упаковке написано другое. Названо — не значит числом: ложка, ломоть, штука, " +
              "тарелка, половина пачки это тоже названное количество, и такое тоже user_text. " +
              "\"package_netto\" — количество не названо, но на упаковке есть масса нетто: считаем, " +
              "что съедена вся пачка. \"photo_estimate\" — количество видно по снимку еды. " +
              "\"default_100\" — узнать неоткуда НИ ОТКУДА, и тогда в граммах стоит ровно 100 и " +
              "никогда ничего другого; поставил сюда иное число — значит источник был не этот.",
            type: "string",
...
const CASES = [
  ...
  // ── снимок еды + описание: описание правит имя ──
  //
  // Зрение на тарелке называет разряд («Мясное блюдо») и занижает вес вдвое —
  // и то и другое воспроизводится побайтово, это предел модели, а не разброс.
  // Чинит это человек, словами. Главная опасность здесь не в цифрах: нельзя,
  // чтобы «печень» из описания стала ТРЕТЬЕЙ позицией рядом с «мясным блюдом»
  // со снимка. Это одно и то же, и свести их — работа этого прохода.
  {
    what: "тарелка + описание называет оба блюда и оба веса",
    frames: ["food-liver-cauliflower.jpg"],
    description: "300 г печени со сливками и примерно 400 г цветной капусты",
    want: [
      { key: ["печен"], grams: 300, source: "user_text" },
      { key: ["капуст"], grams: 400, source: "user_text" },
    ],
  },
  {
    what: "тарелка + описание правит только имя мяса, без весов",
    frames: ["food-liver-cauliflower.jpg"],
    description: "это говяжья печень, тушённая в сливках",
    want: [
      { key: ["печен"], grams: [100, 200], source: "photo_estimate" },
      { key: ["капуст"], grams: [150, 250], source: "photo_estimate" },
    ],
  },
  {
    what: "тарелка + описание про одно блюдо из двух",
    frames: ["food-liver-cauliflower.jpg"],
    description: "печени тут 300 грамм",
    want: [
      { key: ["печен"], grams: 300, source: "user_text" },
      { key: ["капуст"], grams: [150, 250], source: "photo_estimate" },
    ],
  ...
  {
    what: "тарелка + описание добавляет то, чего на снимке не видно",
    frames: ["food-liver-cauliflower.jpg"],
    description: "печень со сливками 300 г, цветная капуста 400 г, и ломоть хлеба",
    want: [
      { key: ["печен"], grams: 300, source: "user_text" },
      { key: ["капуст"], grams: 400, source: "user_text" },
      { key: ["хлеб"], grams: [20, 80] },
    ],
  },
  {
