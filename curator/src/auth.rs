//! Вход куратора паскеем.
//!
//! Тот же auth-worker, что у приложения худеющего и у консоли, но СВОЙ RP:
//! кураторское приложение живёт на curator.renorma.app, и rp_id там не является
//! registrable suffix ни для fit.renorma.app, ни для admin.renorma.app. Три
//! области ключей не пересекаются структурно, а не только по origin.
//!
//! Аппрува здесь нет: куратором становится любой, кто зарегистрировался и позвал
//! /curator/register. Это и есть «свободная регистрация» — в отличие от эксперта,
//! которого оператор одобряет кодом.
//!
//! Разметка WebAuthn перенесена из админки без изменений; сессия — `user_id` и
//! `auth_token` в localStorage, ни базы, ни синка.
...
const KEY_USER_ID: &str = "curator_id";
const KEY_AUTH_TOKEN: &str = "curator_token";
...
/// Завести паскей куратора на этом устройстве. Возвращает новый `sub`; профиль
/// куратора под ним заводит `api::register` — отдельным шагом, потому что
/// личность выдаёт auth-worker, а роль хранит support-worker.
...
/// Войти существующим паскеем (discoverable credential).
