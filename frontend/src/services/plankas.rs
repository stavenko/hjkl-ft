...
//!
//! Одно исключение, и оно про ПРОШЛОЕ, а не про правила: если истории нет ВОВСЕ,
//! кэш заводится от старого места — записи в `goals` для калорий, поля в профиле
//! для шагов; см. [`hydrate`]. Так
...
pub async fn hydrate() {
    ...
    // Планка калорий может лежать ТОЛЬКО в `goals`, без единой записи в истории:
    // так выглядит человек, чья планка старше самой истории, и тот, у кого её
    // смыло — усыновлением устройства, восстановлением базы. Пустая история у
    // него значит не «планки нет», а «журнал пуст», и разница видна сразу:
    // приложение решило бы, что планку пора ПОСТАВИТЬ ВПЕРВЫЕ, и заменило бы его
    // число свежерассчитанным.
    //
    // Поэтому кэш заводится от `goals`, когда истории нет. Обратного правила
    // нет и не нужно: история, если она есть, старше и точнее — она помнит, ЧТО
    // действовало в каждый день.
    if !map.contains_key(Kind::Calories.key()) {
        if let Some(v) = local::legacy_calorie_goal().await {
            map.insert(Kind::Calories.key(), v);
        }
    }
