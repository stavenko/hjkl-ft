// «Мигающие» GIF-подсказки для экрана установки PWA в Яндекс.Браузере на iPhone.
// Тот же росчерковый бурст, что на dashboard-*.gif (см. shot-calcium-highlight.mjs),
// но поверх присланных скриншотов живого интерфейса:
//   1) нижняя панель браузера — кебаб в адресной строке
//   2) меню браузера — «Добавить ярлык на телефон»
//   3) системный лист iOS — «На экран „Домой“»
//   4) диалог iOS — кнопка «Добавить»
import { chromium } from "playwright";
import { execSync } from "node:child_process";
import { mkdirSync, rmSync, readFileSync } from "node:fs";

const DIR = process.argv[2] ||
  "/private/tmp/claude-501/-Users-vasilijstavenko-projects-hjkl-ft/56df53af-a1ed-4117-8e82-8a1f8aad90e8/scratchpad/ios-ya";
...
// то есть половина от присланных ретиновых кадров).

// 1) Нижняя панель Яндекс.Браузера на iPhone: кебаб внутри адресной строки.
await makeGif({ img: "toolbar.jpg", w: 562, h: 230, out: "ios-ya-menu", pad: 8,
  rects: [{ x: 467, y: 44, w: 29, h: 40 }] });

// 2) Меню браузера: строка «Добавить ярлык на телефон».
await makeGif({ img: "menu.jpg", w: 562, h: 137, out: "ios-ya-shortcut", pad: 10,
  rects: [{ x: 35, y: 78, w: 385, h: 38 }] });

// 3) Системный лист iOS: строка «На экран „Домой“».
await makeGif({ img: "sheet.jpg", w: 562, h: 378, out: "ios-ya-home", pad: 8,
  rects: [{ x: 27, y: 138, w: 510, h: 60 }] });

// 4) Диалог iOS: кнопка «Добавить». Кадр без строки состояния телефона —
// часы и батарея к инструкции отношения не имеют и только отвлекают
// (dialog-crop.jpg = dialog.jpg, обрезанный сверху на 140 px).
await makeGif({ img: "dialog-crop.jpg", w: 562, h: 313, out: "ios-ya-add", pad: 8,
  rects: [{ x: 415, y: 38, w: 125, h: 30 }] });
