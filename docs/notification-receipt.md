...
Панель показывает шапку `ver=<билд> ctrl=<y/n>` (версия сборки и наличие
контролирующего service worker) и журнал страницы (`rn_pj` в localStorage) —
события жизненного цикла (`BOOT`, `pageshow`, `focus`, `visibilitychange`) и
шаги доставки (`WASM notif button pressed`, `subscribe ok`, `send_test ok`,
`IDB notif_received …`, `cache notif_received …`, `SW->page notif_received …`,
`WASM poll consumed …`). Журнал намеренно в localStorage, мимо Cache — он
переживает отвал Cache-подключения. Кнопки «Обновить лог» и «Скопировать»
(живое обновление сбрасывает выделение текста — копировать только кнопкой).

Во время расследования дополнительно существовали: SW-лог в Cache с зеркалом
на страницу, «пульсы» таймеров (`rn_hb_js`/`rn_hb_wasm`), детектор
`CACHE-LOG SHRANK` и ping/pong версии SW — сняты после подтверждения фикса
(коммит с этим документом); при рецидиве восстанавливаются по истории git.
