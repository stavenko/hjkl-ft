...
## Версия локальной базы: только вперёд

`DB_VERSION` (`frontend/src/services/db.rs`) **растёт и никогда не падает**. Откат
сборки назад через поднятую версию базы — не вариант, который взвешивают: его нет.
IndexedDB не открывает базу версией МЕНЬШЕ той, что в ней записана, — старый код на
новой базе не запустится вовсе, у всех сразу.

Поэтому:

* **Поднятая версия — не повод предупреждать об откате.** Откатываться некуда, и
  обсуждать нечего. Чинится всё только новой сборкой вперёд, с новой версией и, если
  данные того требуют, с миграцией (`frontend/src/services/migrations/`).
* Поломку в выложенной сборке чинит **следующая выкладка**, а не возврат предыдущей.
* Если понадобилось отменить смысл прошлой миграции — это НОВАЯ миграция с бо́льшим
  номером, которая приводит данные обратно. Не откат.

Новый пустой store миграции не требует: он объявляется в билдере, дописывается в
`ALL_STORES`, версия поднимается — `idb` создаёт его сам при открытии. Миграция нужна
там, где надо ПЕРЕЛОЖИТЬ уже лежащие данные.
