...
const S8: &[Frame] = &[
    ...
    // 11 — врачебное назначение важнее нашей планки. Показано, КУДА писать и ЧТО:
    // готовое сообщение в ленте чата и подсвеченная кнопка поддержки в нижнем меню.
    //
    // Снято на КОРОТКОМ экране, одним куском. Первая попытка резала длинный экран
    // на две полосы — сообщение и меню — и склеивала их: на стыке ломался градиент
    // обоев чата, и шов было видно.
    Frame {
        bg: Bg::Dark,
        media: Media::ShotTop("eggs-chat-highlight.gif"),
        accent: AMBER,
        kicker: Loc {
...
const S9: &[Frame] = &[
    // 1 — поздравление: яйца позади, впереди клетчатка. Такой кадр открывает каждую
    // главу, и эта не исключение — меняется только название темы в градиенте.
    Frame {
        bg: Bg::Dark,
        media: Media::Emoji("🎉"),
        accent: GREEN,
        ...
        title: Loc { en: "", ru: "" },
        body: Loc {
            en: "You have dealt with the egg week. Eggs are back where they belong — among \
                 ordinary food, without the fear that surrounded them for half a century.\n\n\
                 And we move on towards a healthy diet. The next week is the ^fibre week^.",
            ru: "Вы справились с неделей яиц. Яйца вернулись туда, где им и место, — в обычную \
                 еду, без страха, который окружал их полвека.\n\nА мы двигаемся дальше к \
                 здоровому рациону. И следующая неделя это ^неделя клетчатки^.",
        },
    },
    // 2 — виды клетчатки. Зелень всех сортов разом: и листовая, и капустные, и
    // семена — кадр про то, что клетчатка бывает разной, а в еде идёт вперемешку.
    Frame {
        bg: Bg::Dark,
        media: Media::Cover("fiber-greens.jpg"),
        accent: GREEN,
        kicker: Loc { en: "Fibre", ru: "Клетчатка" },
    ...
    // (огурец, помидор, салат) — ровно то противопоставление, о котором текст.
    Frame {
        bg: Bg::Dark,
        media: Media::Cover("fiber-vegetables.jpg"),
        accent: GREEN,
        kicker: Loc { en: "Fibre", ru: "Клетчатка" },
    ...
    // 4 — ягоды. Малина, ежевика и смородина — те самые, что названы в тексте.
    Frame {
        bg: Bg::Dark,
        media: Media::Cover("fiber-berries.jpg"),
        accent: GREEN,
        kicker: Loc { en: "Fibre", ru: "Клетчатка" },
    ...
    // 5 — фрукты. Целые плоды, а не сок и не сухофрукты: кадр держит ту же мысль,
    // что и текст, — клетчатка остаётся во фрукте, пока его не отжали.
    Frame {
        bg: Bg::Dark,
        media: Media::Cover("fiber-fruit.jpg"),
        accent: GREEN,
        kicker: Loc { en: "Fibre", ru: "Клетчатка" },
    ...
    // 6 — бобовые. Плошки с чечевицей, фасолью и нутом: кадр про самый плотный
    // источник, и разнообразие здесь — часть довода.
    Frame {
        bg: Bg::Dark,
        media: Media::Cover("fiber-legumes.jpg"),
        accent: GREEN,
        kicker: Loc { en: "Fibre", ru: "Клетчатка" },
    ...
    // 7 — вздутие. Шарик у живота: кадр про то, чего люди боятся, начав есть больше
    // клетчатки, — и про то, что это проходит.
    //
    // ЦЕЛИКОМ, а не `Cover`: `Cover` растягивает снимок на 145 % ширины от центра и
    // срезает по краям — шарик уходил за левый экранный край.
    //
    // Снимок ОБРЕЗАН: в стоке фигура стоит вплотную к левому краю, справа треть
    // кадра пустого фона, а снизу — бельё и бёдра, из-за которых кадр читался как
    // ню, а не как рассказ про живот. Осталось 700×470 от левого верхнего угла:
    // шарик и рука на животе, поля по бокам сравнялись. Файл переименован, чтобы
    ...
    // поворот ничего не ломает — ни теней, ни горизонта в кадре нет.
    Frame {
        bg: Bg::Dark,
        media: Media::Cover("fiber-grains.jpg"),
        accent: GREEN,
        kicker: Loc { en: "Fibre", ru: "Клетчатка" },
