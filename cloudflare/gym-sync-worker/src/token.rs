...
/// Ported verbatim from payment-worker/src/token.rs (verify path only).
...
/// Signature-only — sync-worker has no AuthDO, so there is no revocation check.
/// Returns the authenticated subject (`claims.sub`), used as the SyncDO name.
